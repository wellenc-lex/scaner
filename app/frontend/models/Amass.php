...
class Amass extends ActiveRecord
{
    public static function amassscan($input)
    {
        ...
        $command = ("sudo docker run --net=host --cpu-shares 32 --rm -v configs:/configs/ -v dockerresults:/dockerresults caffix/amass enum -w /configs/amass/amasswordlistALL2.txt -d  " . escapeshellarg($url) . " -json " . $enumoutput . " -active -brute -timeout 3500 -ip -config ".$amassconfig);
