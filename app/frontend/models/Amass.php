...
//--net=host
        $command = ("sudo docker run  --cpu-shares 256 --rm -v configs:/configs/ -v dockerresults:/dockerresults caffix/amass:v3.23.3 enum -w /configs/amass/amasswordlistASSETNOTE -d  " . escapeshellarg($url) . " -json " . $enumoutput . " -active -brute -timeout 4500 -ip -config ".$amassconfig);
...
{\"name\"", "},{\"name\"", $fileamass);
    ...
    public static function bannedwords($in)
    {
        if (preg_match("/dev|stage|test|proxy|stg|int|adm|uat/i", $in) === 1) {
            ...
            return preg_match("/sentry|support|^ws|wiki|status|socket|url(\d)*/i", $in);
        }
    }
...
*/
    ...
    public static function httpxhosts($vhostslist, $taskid, $randomid)
    {
        ...
        //--net=container:vpn1
        $httpx = "sudo docker run --net=host --cpu-shares 512 --rm -v dockerresults:/dockerresults projectdiscovery/httpx -ports 80,81,443,8080,8443,8000,3000,8083,8088,8888,8880,8083,9999,10000,10250,4443,6443,8123,8000,2181,9092,9200,9100,9080,9443 -rate-limit 30 -timeout 30 -threads 30 -retries 3 -silent -o ". $output ." -l ". $wordlist ." -json -tech-detect -title -favicon -ip -sr -srd ". $httpxresponsesdir;
