...
//--net=container:vpn2 -hang-monitor
        $nuclei_start = "sudo docker run --rm --cpu-shares 512 -v nuclei:/nuclei -v configs:/root/ projectdiscovery/nuclei -t /root/nuclei/ -w /root/nuclei-templates/workflows -t /root/nuclei-templates/ -w /root/nuclei-templates/workflows -t /root/nuclei-custom/5.log4j.yaml -list " . escapeshellarg($list) . " -o " . $output . " -j -irr -max-host-error 200 -timeout 130 -rl 10 -bs 200 -c 1 -hbs 3 -stats -retries 3 -error-log /nuclei/error.log -page-timeout 120 -ztls -disable-update-check -system-resolvers  " . $exclude . $headers;
