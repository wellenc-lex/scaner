...
class Nuclei extends ActiveRecord
{
    ...
    public static function bannedsubdomains($in)
    {
        if (preg_match("/dev|stage|test|proxy|stg|int|adm|uat/i", $in) === 1) {
            ...
            return preg_match("/link|cdn|sentry|status|socket|help|autodiscover|url(\d)*/i", $in);
        }
    }
...
//--net=container:vpn1 -hang-monitor --net docker_default
        $nuclei_start = "sudo docker run --rm --cpu-shares 24 -v nuclei:/nuclei -v configs:/root/ projectdiscovery/nuclei -t /root/nuclei-custom/ -t /root/nuclei/ -t /root/nuclei-templates/ -w /root/nuclei-templates/workflows -list " . escapeshellarg($list) . " -o " . $output . " -j -irr -max-host-error 200 -timeout 60 -rl 2 -bs 50 -ss host-spray -stats -retries 3 -error-log /nuclei/error.log -page-timeout 80 -ztls -disable-update-check -system-resolvers -code  " . $exclude . $headers;

//-ept network -silent -stats
        /*$nuclei_start = "sudo /root/bin/bin/nuclei -t /root/nuclei-templates/ -list " . escapeshellarg($list) . " -o " . $output . " -json -irr -retries 2 -max-host-error 50 -timeout 180 -headless -code -silent -rl 25 -bs 2000 -c 25 -hbs 55 " . $exclude . $headers;  //-stats*/
