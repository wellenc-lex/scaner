...
{\"name\"", "},{\"name\"", $fileamass);
    ...
    public static function httpxhosts($vhostslist, $taskid, $randomid)
    {
        ...
        //--net=container:vpn1
        $httpx = "sudo docker run --cpu-shares 512 --rm -v dockerresults:/dockerresults projectdiscovery/httpx -ports 80,443,8080,8443,8000,3000,8083,8088,8888,2379,8880,9999,10000,10250,4443,6443,10255,2379,6666,8123,8000,2181,9092 -rate-limit 15 -timeout 35 -threads 50  -retries 3 -silent -o ". $output ." -l ". $wordlist ." -json -tech-detect -title -favicon -ip -sr -srd ". $httpxresponsesdir;
