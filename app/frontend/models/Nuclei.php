...
class Nuclei extends ActiveRecord
{
    ...
    {
        $exclude = " -exclude-templates /root/nuclei-templates/fuzzing -exclude-templates /root/nuclei-templates/dns -exclude-templates /root/nuclei-templates/miscellaneous -exclude-templates /root/nuclei-templates/technologies/tech-detect.yaml -exclude-templates /root/nuclei-templates/technologies/aws -exclude-templates /root/nuclei-templates/technologies/waf-detect.yaml -exclude-templates /root/nuclei-templates/misconfiguration/http-missing-security-headers.yaml -exclude-templates /root/nuclei-templates/misconfiguration/cloudflare-image-ssrf.yaml -exclude-templates /root/nuclei-templates/cves/2018/CVE-2018-15473.yaml -exclude-templates /root/nuclei-templates/vulnerabilities/generic/cors-misconfig.yaml -exclude-templates /root/nuclei-templates/cves/2018/CVE-2017-5487.yaml -exclude-templates /root/nuclei-templates/cves/2018/CVE-2021-38314.yaml -exclude-templates /root/nuclei-templates/cves/2016/CVE-2016-10940.yaml -exclude-templates /root/nuclei-templates/cves/2016/CVE-2017-5487.yaml -exclude-templates /root/nuclei-templates/vulnerabilities/generic/open-redirect.yaml -exclude-templates /root/nuclei-templates/ssl/deprecated-tls.yaml -exclude-templates /root/nuclei/Custom-Nuclei-Templates/shells.yaml 
     -exclude-templates /root/nuclei/Custom-Nuclei-Templates/header_sqli.yaml -exclude-templates /root/nuclei/Custom-Nuclei-Templates/templates/sqli_header.yaml -exclude-templates /root/nuclei/header_reflection_body.yaml -exclude-templates /root/nuclei-templates/technologies/nginx/nginx-version.yaml 
"; // -exclude-templates /root/nuclei-templates/helpers -exclude-templates /root/nuclei-templates/token-spray/ -exclude-templates /root/nuclei-templates/exposures/tokens/generic/ -etags xss 

        //-exclude-severity info

        $output = "/nuclei/" . $randomid . "/" . $randomid . "out.json";
//
        $nuclei_start = "sudo docker run  --net=container:vpn2 --rm --cpu-shares 512 -v nuclei:/nuclei -v configs:/root/ projectdiscovery/nuclei -t /root/nuclei/ -t /root/nuclei-templates/ -w /root/nuclei-templates/workflows -t /root/nuclei-custom/5.log4j.yaml -list " . escapeshellarg($list) . " -o " . $output . " -json -irr -max-host-error 100 -timeout 130 -rl 2 -bs 50 -c 3 -hbs 5 -fr -mr 5 -stats -retries 3 -error-log /nuclei/error.log -page-timeout 120 -hang-monitor -ztls -disable-update-check -system-resolvers -resolvers /root/resolv.conf " . $exclude . $headers;
...
//-ept network -silent -stats
        ...
        exec("echo " . $nuclei_start . " >> /dockerresults/out.txt ");
