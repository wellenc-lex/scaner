...
//--net=container:vpn". rand(1,3) ."
    ...
    public static function bannedsubdomains($in)
    {
        if (preg_match("/dev|stage|test|proxy|stg|int|adm|uat/i", $in) === 1) {
            ...
            return preg_match("/link|support|^ws|wiki|enterpriseenrollment|status|docs|help|jira|lync|maintenance|atlassian|autodiscover|grafana|confluence|spider-(\d)*.yandex.*|gitlab|zendesk|sentry|(url(\d)*)|.*\.gb\.ru/i", $in);
