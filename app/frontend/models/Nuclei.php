...
//--net=container:vpn1 -hang-monitor
        $nuclei_start = "sudo docker run  --rm --cpu-shares 512 -v nuclei:/nuclei -v configs:/root/ projectdiscovery/nuclei -t /root/nuclei/ -t /root/nuclei-templates/ -w /root/nuclei-templates/workflows -t /root/nuclei-custom/5.log4j.yaml -list " . escapeshellarg($list) . " -o " . $output . " -j -irr -max-host-error 200 -timeout 80 -rl 5 -bs 250 -c 4 -hbs 4 -stats -retries 3 -error-log /nuclei/error.log -page-timeout 80 -ztls -disable-update-check -system-resolvers  " . $exclude . $headers;
